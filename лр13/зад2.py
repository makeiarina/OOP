...
btn=Button(window,text='Приветствие',font=('Arial',14),command=clicked)#создает Button с текстом “Приветствие” и устанавливает шрифт Arial размером 14. При нажатии на кнопку будет вызываться функция clicked()
btn.grid(column=0, row=1)#размещает Button в главном окне в первом столбце и второй строке

btn1=Button(window,text='Закрыть',font=('Arial',14),command=close_window)#создает Button с текстом “Закрыть” и устанавливает шрифт Arial размером 14. При нажатии на кнопку будет вызываться функция close_window()
